...
class XorGate : public Circuit<2,0,0,1,0,0> {
  public:
    XorGate() {
      this->init();
      this->components_.push_back(
		std::make_shared<Nor>(	(this->bit_cin_[0])->release_output(0),
					(this->bit_cin_[1])->release_output(0) ) );
      this->components_.push_back(
		std::make_shared<And>(	(this->bit_cin_[0])->release_output(0),
					(this->bit_cin_[1])->release_output(0) ) );
      this->components_.push_back(
		std::make_shared<Nor>(	( this->components_[0] )->release_output(0),
