// 20a - Logic Engine
#include "circuit.hpp"
#include "word_splitter.hpp"
#include "word_or.hpp"
#include "word_not.hpp"
#include "toggledword.hpp"
#include "threebitdecoder.hpp"
#include <sstream>
#include <iostream>

using namespace Virtual;

class LogicEngine : public Circuit<0,3,0,0,1,0> {
  public:
    LogicEngine() {
      this->init();
      this->components_.push_back(
		std::make_shared<ByteSplitter>(
			(this->byte_cin_[0])->release_byte_output(0)));
      this->components_.push_back(
		std::make_shared<ByteOr>(
			(this->byte_cin_[1])->release_byte_output(0),
			(this->byte_cin_[2])->release_byte_output(0)));
      // Components 2:
      this->components_.push_back(
		std::make_shared<ByteNot>(
			(this->byte_cin_[1])->release_byte_output(0)));
      this->components_.push_back(
		std::make_shared<ByteNot>(
			(this->byte_cin_[2])->release_byte_output(0)));
      this->components_.push_back(
		std::make_shared<ByteOr>(
			(this->components_[2])->release_byte_output(0),
			(this->components_[3])->release_byte_output(0)));
      // Components 5:
      this->components_.push_back(
		std::make_shared<ThreeBitDecoder>() );
      this->components_[5]->connect_input(
			(this->components_[0])->release_output(0), 0);
      this->components_[5]->connect_input(
			(this->components_[0])->release_output(1), 1);
      // Components 6:
      this->components_.push_back(
		std::make_shared<ToggledByte>(
			(this->components_[1])->release_byte_output(0),
			(this->components_[5])->release_output(0)));
      this->components_.push_back(
		std::make_shared<ByteNot>(
			(this->components_[1])->release_byte_output(0)));
      // Components 8:
      this->components_.push_back(
		std::make_shared<ToggledByte>(
			(this->components_[7])->release_byte_output(0),
			(this->components_[5])->release_output(2)));
      this->components_[8]->connect_byte_output(
		this->components_[6]->release_byte_output(0));
      // Components 9:
      this->components_.push_back(
		std::make_shared<ToggledByte>(
			(this->components_[4])->release_byte_output(0),
			(this->components_[5])->release_output(1)));
      this->components_[9]->connect_byte_output(
		this->components_[6]->release_byte_output(0));
      // Components 10:
      this->components_.push_back(
		std::make_shared<ByteNot>(
			(this->components_[4])->release_byte_output(0)));
      // Components 11:
      this->components_.push_back(
		std::make_shared<ToggledByte>(
			(this->components_[10])->release_byte_output(0),
			(this->components_[5])->release_output(3)));
      this->components_[11]->connect_byte_output(
		this->components_[6]->release_byte_output(0));
      this->byte_cout_[0]->connect_byte_input(
			this->components_[6]->release_byte_output(0), 0);
    }
};
      

int main() {

std::stringstream buffer;
std::streambuf* old = std::cout.rdbuf(buffer.rdbuf());

LogicEngine circuit = LogicEngine();
std::vector<std::uint8_t> input{3, 17, 178};
circuit.tick(input);
input[0] = 2;
input[1] = 33;
input[2] = 200;
circuit.tick(input);
input[0] = 3;
input[1] = 179;
input[2] = 155;
circuit.tick(input);
input[0] = 0;
input[1] = 34;
input[2] = 41;
circuit.tick(input);
input[1] = 226;
input[2] = 4;
circuit.tick(input);
input[0] = 1;
input[1] = 132;
input[2] = 180;
circuit.tick(input);
input[0] = 0;
input[1] = 133;
input[2] = 171;
circuit.tick(input);
input[0] = 3;
input[1] = 84;
input[2] = 151;
circuit.tick(input);
input[0] = 2;
input[1] = 28;
input[2] = 111;
circuit.tick(input);
input[0] = 0;
input[1] = 84;
input[2] = 37;
circuit.tick(input);
input[1] = 0;
input[2] = 77;
circuit.tick(input);
input[0] = 3;
input[1] = 11;
input[2] = 251;
circuit.tick(input);
input[0] = 1;
input[1] = 54;
input[2] = 9;
circuit.tick(input);
input[0] = 3;
input[1] = 142;
input[2] = 236;
circuit.tick(input);
input[1] = 73;
input[2] = 157;
circuit.tick(input);

std::cout.rdbuf(old);

std::string capturedOutput = buffer.str();
std::string expectedOutput = std::string("b16 b22 b147 b43 b230 b123 ") +
			"b175 b20 b128 b117 b77 b11 b255 b140 b9 ";

if (capturedOutput != expectedOutput) {
  std::cerr << "Expected: " << expectedOutput << std::endl;
  std::cerr << "Output: " << capturedOutput << std::endl;
  throw std::logic_error("Test failed!");
}
}
