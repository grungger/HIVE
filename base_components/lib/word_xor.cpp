...
ByteXor::ByteXor(ptr8_t inA, ptr8_t inB) {
  ...
  ByteArg argC;
  for (int i=0; i<8; i++) {
    circuit_components_[i+2] = std::make_shared<Xor>(
...
const std::string ByteXor::gate_name = "ByteXor";


WordXor::WordXor(ptr32_t inA, ptr32_t inB) {
...
const std::string WordXor::gate_name = "WordXor";
