...
ByteMUX::ByteMUX(ptr_t toggler, ptr8_t  inA, ptr8_t inB) {
  ...
  circuit_components_[1] = std::make_shared<ToggledByte>(inA,
...
const std::string ByteMUX::gate_name = "ByteMUX";


WordMUX::WordMUX(ptr_t toggler, ptr32_t inA, ptr32_t inB) {
  input_pointers_[0] = toggler;
  word_input_pointers_[0] = inA;
  word_input_pointers_[1] = inB;
  circuit_components_[0] = std::make_shared<Not>(toggler);
  circuit_components_[1] = std::make_shared<ToggledWord>(inA,
				circuit_components_[0]->release_output(0));
...
void WordMUX::rewire_input() {
  ...
  circuit_components_[2]->connect_word_input(word_input_pointers_[1],0);
  circuit_components_[2]->connect_input(input_pointers_[0],0);
}

const std::string WordMUX::gate_name = "WordMUX";
