 ...
 * 
 * ALU_HIVE implements the following operations: Not, And, Nand, Or, Nor, Xor, BIC, Add, AddC, Sub, SubC,
 * 							RevSub, RevSubC, ArithShiftR, LogShiftR, LogShiftL, (ROR)
 * Unary operations are computed on inputA.
 * The output byte corresponds to the carry byte with bit 128 for ADC, bit 64 for SBC & RSC borrow, bit 32 for SBC & RSC overflow.
 */
class ALU_HIVE : public wordMicroCircuit<1,2,2,0,1,1,TODO> {
  public:
    /**
